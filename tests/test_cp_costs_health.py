...
class TestCostRecording:
    ...
    def test_record_multiple_costs(self, db, workspace):
        record_cost(
            db, workspace_id=workspace.id, provider_key="claude",
            cost_unit="tokens", quantity=500, usd_equivalent=0.0015
        )
        record_cost(
            db, workspace_id=workspace.id, provider_key="elevenlabs",
            cost_unit="characters", quantity=200, usd_equivalent=0.002
        )
...
class TestBudgetPolicies:
    ...
    def test_warning_when_near_limit(self, db, workspace):
        set_budget(
            db, scope="workspace", scope_id=workspace.id,
            period="monthly", limit_usd=1.0, actor="cli",
            warning_threshold=0.5,
        )
        record_cost(
            db, workspace_id=workspace.id, provider_key="claude",
            cost_unit="usd", quantity=1, usd_equivalent=0.6
        )
        result = check_budget(db, workspace.id)
    ...
    def test_block_action_raises_when_exceeded(self, db, workspace):
        set_budget(
            db, scope="workspace", scope_id=workspace.id,
            period="monthly", limit_usd=0.001, actor="cli",
            on_exceed_action="block",
        )
        record_cost(
            db, workspace_id=workspace.id, provider_key="claude",
            cost_unit="usd", quantity=1, usd_equivalent=0.5
        )
    ...
    def test_warn_action_does_not_raise(self, db, workspace):
        ...
        record_cost(
            db, workspace_id=workspace.id, provider_key="claude",
            cost_unit="usd", quantity=1, usd_equivalent=1.0
        )
        result = check_budget(db, workspace.id)
        assert result["ok"]
...
class TestHealthManagement:
    ...
    def test_latest_record_returned(self, db):
        record_health(
            db, entity_type="provider", entity_id="claude",
            status="healthy", recorded_by="monitor"
        )
        record_health(
            db, entity_type="provider", entity_id="claude",
            status="degraded", recorded_by="monitor", detail="High latency"
        )
    ...
    def test_list_degraded_entities(self, db):
        record_health(
            db, entity_type="platform_account", entity_id="acc-1",
            status="unavailable", recorded_by="monitor"
        )
        record_health(
            db, entity_type="channel", entity_id="ch-2",
            status="healthy", recorded_by="monitor"
        )
    ...
    def test_multiple_degraded_entities(self, db):
        for i in range(3):
            record_health(
                db, entity_type="platform_account", entity_id=f"acc-{i}",
